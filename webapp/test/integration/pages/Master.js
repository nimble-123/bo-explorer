				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("NodeKey") === oObject.NodeKey;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].NodeKey;
										} else {
											sCurrentId = aItemsNotInTheList[0].NodeKey;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("BusinessObjects", {
											NodeKey : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("NodeKey"),
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "BusinessObjects (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'BusinessObjects (0)'");
							},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all BusinessObjects to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "BusinessObjects (" + iExpectedLength + ")"}),
									success : function () {

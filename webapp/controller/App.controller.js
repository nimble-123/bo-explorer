...
sap.ui.define([
	...
	"sap/m/MessageBox",
	"sap/m/library"
], function(jQuery, BaseController, JSONModel, ResizeHandler, Device, Fragment, library, IconPool, SplitAppMode, MessageBox,
	mobileLibrary) {
	"use strict";
	...
	return BaseController.extend("dev.bo-explorer.controller.App", {
		...
		onRouteNotFound: function() {
			this.getRouter().myNavToWithoutHash("dev.bo-explorer.view.NotFound", "XML", false);
			return;
		},
		...
		aboutDialogOpen: function() {
			if (!this._oAboutDialog) {
				this._oAboutDialog = new sap.ui.xmlfragment("aboutDialogFragment", "dev.bo-explorer.view.AboutDialog", this);
		...
		onReleaseDialogOpen: function(oEvent) {
			...
			if (!this._oReleaseDialog) {
				this._oReleaseDialog = new sap.ui.xmlfragment("releaseDialogFragment", "dev.bo-explorer.view.ReleaseDialog", this);
			...
			if (!this._oNotesView) {
				this._oNotesView = sap.ui.view({
					id: "notesView",
					viewName: "dev.bo-explorer.view.ReleaseNotesView",
					type: "Template"
				});
		...
		feedbackDialogOpen: function() {
			...
			if (!this._oFeedbackDialog) {
				this._oFeedbackDialog = new sap.ui.xmlfragment("feedbackDialogFragment", "dev.bo-explorer.view.FeedbackDialog", this);
		...
		onFeedbackDialogSend: function() {
			...
			jQuery.ajax({
				...
				function() {
					MessageBox.success("Your feedback has been sent.", {
						title: "Thank you!"
					});
